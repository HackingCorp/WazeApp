...
// helpers must be loaded via dynamic import (mirroring baileys.service.ts).
// loadBridgeDeps() must be awaited once before calling makeBrowserAuthImport().
let jidDecode: any;
let jidEncode: any;
let initAuthCreds: any;
let Curve: any;
let generateSignalPubKey: any;
let BufferJSON: any;

export const loadBridgeDeps = async (): Promise<void> => {
